...
import com.awesomesauce.minecraft.forge.core.lib.item.TRedstonePulseActivated
...
class TileEntityResearcher extends TileEntity with IInventory with TActivatedTileEntity with TRedstonePulseActivated {
  ...
  def getStackInSlot(slot: Int): ItemStack = try {inventory(slot)} catch {case e:NoSuchElementException =>  null}
  ...
  def tryToCraft() = {
    for (i <- RnDTechAPI.getResearchIterator) {
      if ((!i.isResearchDone(getStackInSlot(0))) && i.getRecipe.doesItemMatch(getStackInSlot(1))) {
        val block = WorldUtil.scanForBlockTileEntityInstanceOf[IInventory](worldObj, new PositionImpl(xCoord, yCoord, zCoord))
        for (x <- block) {
          val te = worldObj.getTileEntity(x.getX().toInt, x.getY().toInt, x.getZ().toInt).asInstanceOf[IInventory]
